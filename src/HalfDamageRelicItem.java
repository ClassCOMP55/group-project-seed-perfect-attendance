import acm.graphics.GImage;

import java.awt.image.BufferedImage;
import java.io.File;
import java.io.IOException;
import javax.imageio.ImageIO;

/**
 * The Half-Damage (Health) relic item.
 * Received from a chest; passively cuts all incoming damage in half.
 * Non-consumable — passive effect, not consumed on use.
 */
public class HalfDamageRelicItem extends Item {

    public static final String ITEM_ID   = "relic_half_damage";
    private static final double ICON_SIZE = 32.0;

    public HalfDamageRelicItem() {
        super(ITEM_ID, "Relic of Strength", false);
        BufferedImage trimmed = loadTrimmed("assets/visuals/png's/health_relic.png");
        if (trimmed != null) {
            icon = new GImage(trimmed);
            icon.setSize(ICON_SIZE, ICON_SIZE);
        }
    }

    @Override
    public String getDescription() {
        return "A circlet earned through the Trial of Strength, fit for a hero who can take a hit. Your body is tougher than it looks now, capable of taking twice the punishment it could before.";
    }

    private static BufferedImage loadTrimmed(String path) {
        try {
            BufferedImage source = ImageIO.read(new File(path));
            if (source == null) return null;
            int w = source.getWidth(), h = source.getHeight();
            int minX = w, minY = h, maxX = -1, maxY = -1;
            for (int py = 0; py < h; py++) {
                for (int px = 0; px < w; px++) {
                    if (((source.getRGB(px, py) >>> 24) & 0xFF) == 0) continue;
                    if (px < minX) minX = px;
                    if (py < minY) minY = py;
                    if (px > maxX) maxX = px;
                    if (py > maxY) maxY = py;
                }
            }
            return (maxX < minX || maxY < minY) ? source
                : source.getSubimage(minX, minY, maxX - minX + 1, maxY - minY + 1);
        } catch (IOException | RuntimeException ignored) {
            return null;
        }
    }
}
